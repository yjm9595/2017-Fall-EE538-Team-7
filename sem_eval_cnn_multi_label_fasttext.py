...
prob_data_norm = KeyedVectors.load_word2vec_format('word_naive_bayes_probability_normalized.txt')
...
#        embedded_sentence_data[-1] += list(prob_data_norm.wv[word.lower()])
...
if __name__ == "__main__":
    ...
    with open("sem_eval_slot1_epoch_{}_hidden_{}_filter_{}_adam_select_200_batch_50_withoutNB.txt".format(n_epochs, n_hidden, n_filter), "w") as f:
        index = 0
        for text_data, category_data in zip(text_data_lst[TRAINING_INDEX:], category_data_lst[TRAINING_INDEX:]):
            index += 1
            print(index, "/ 685")
...
#                embedded_sentence_data[-1] += NER_dict[tag[1]]
 ...
 #           pred_nil = model0.predict(embedded_sentence_data)
